...
public enum DeviceType: Int, CaseIterable, Identifiable, Codable, Sendable {
    case serial = 0
    case bluetooth = 1
    case tcp = 2
...
public class DeviceConnection: Identifiable, ObservableObject {
    public let id = UUID()
...
public class AppState: ObservableObject {
    @Published public var availableDevices: [DeviceInfo] = [] {
        didSet {
            print("[AppState] availableDevices updated: \(availableDevices.count) devices")
            NSLog("[AppState] availableDevices updated: %d devices", availableDevices.count)
            for (index, device) in availableDevices.enumerated() {
                print("[AppState]   Device \(index): \(device.name)")
                NSLog("[AppState]   Device %d: %@", index, device.name)
            }
        }
    }
    ...
    @Published public var isScanning: Bool = false {
        didSet {
            print("[AppState] isScanning updated: \(isScanning)")
            NSLog("[AppState] isScanning updated: %@", isScanning ? "YES" : "NO")
        }
    }
